...
namespace FlatStage.Foundation.TinyAudio;

/// <summary>
/// Implements a background audio playback stream.
/// </summary>
public abstract class PlatformAudioPlayer : IDisposable
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="PlatformAudioPlayer"/> class.
    /// </summary>
    /// <param name="format">Format of the audio stream.</param>
    protected PlatformAudioPlayer(AudioFormat format)
    {
    ...
    /// <exception cref="ObjectDisposedException">The <see cref="PlatformAudioPlayer"/> instance has been disposed.</exception>
    ...
    {
        if (this.disposed)
            throw new ObjectDisposedException(nameof(PlatformAudioPlayer));
        if (this.Playing)
    ...
    public ValueTask WriteDataAsync(ReadOnlyMemory<float> data, CancellationToken cancellationToken = default) =>
        this.writer.WriteDataAsync(data, cancellationToken);
    ...
    /// <param name="data">Buffer containing data to write.</param>
    /// <param name="cancellationToken">Token used to cancel asynchronous operation.</param>
    public ValueTask WriteDataAsync(ReadOnlyMemory<short> data, CancellationToken cancellationToken = default) =>
        this.writer.WriteDataAsync(data, cancellationToken);
    ...
    /// <param name="data">Buffer containing data to write.</param>
    /// <param name="cancellationToken">Token used to cancel asynchronous operation.</param>
    public ValueTask WriteDataAsync(ReadOnlyMemory<byte> data, CancellationToken cancellationToken = default) =>
        this.writer.WriteDataAsync(data, cancellationToken);
    ...
    /// </list>
    /// </remarks>
    public ValueTask WriteDataRawAsync<TSample>(ReadOnlyMemory<byte> data,
        CancellationToken cancellationToken = default) where TSample : unmanaged =>
        this.writer.WriteDataRawAsync<TSample>(data, cancellationToken);
    ...
    protected void RaiseCallback(Span<byte> buffer, out int samplesWritten) =>
        this.RaiseCallbackInternal(buffer, out samplesWritten);

    protected void RaiseCallback(Span<short> buffer, out int samplesWritten) =>
        this.RaiseCallbackInternal(buffer, out samplesWritten);

    protected void RaiseCallback(Span<float> buffer, out int samplesWritten) =>
        this.RaiseCallbackInternal(buffer, out samplesWritten);
    ...
    private async ValueTask WriteDataInternalAsync<T>(ReadOnlyMemory<T> data, CancellationToken cancellationToken)
        where T : unmanaged
    {
        int bytesWritten = 0;
    ...
    private async ValueTask WriteDataRawInternalAsync<T>(ReadOnlyMemory<byte> data, CancellationToken cancellationToken)
        where T : unmanaged
    {
        int bytesWritten = 0;
        int byteLength = Unsafe.SizeOf<T>() * data.Length;
    ...
    private abstract class InternalBufferWriter
    {
        ...
        public abstract ValueTask WriteDataAsync<TInput>(ReadOnlyMemory<TInput> data,
            CancellationToken cancellationToken) where TInput : unmanaged;

        public abstract ValueTask WriteDataRawAsync<TInput>(ReadOnlyMemory<byte> data,
            CancellationToken cancellationToken) where TInput : unmanaged;
    }
    ...
    {
        private readonly PlatformAudioPlayer player;
        private TOutput[]? conversionBuffer;

        public InternalBufferWriter(PlatformAudioPlayer player) => this.player = player;
        ...
        public override int WriteData<TInput>(ReadOnlySpan<TInput> data)
        {
            ...
            return this.player.WriteDataInternal(MemoryMarshal.AsBytes(this.conversionBuffer.AsSpan(0, data.Length))) /
                   Unsafe.SizeOf<TOutput>();
        }

        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        public override ValueTask WriteDataAsync<TInput>(ReadOnlyMemory<TInput> data,
            CancellationToken cancellationToken)
        {
            ...
            return this.player.WriteDataInternalAsync<TOutput>(this.conversionBuffer.AsMemory(0, data.Length),
                cancellationToken);
        }

        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        public override ValueTask WriteDataRawAsync<TInput>(ReadOnlyMemory<byte> data,
            CancellationToken cancellationToken)
        {
            // if formats are the same no sample conversion is needed
            if (typeof(TInput) == typeof(TOutput))
            ...
            SampleConverter.InternalConvert<TInput, TOutput>(MemoryMarshal.Cast<byte, TInput>(data.Span),
                this.conversionBuffer);
            return this.player.WriteDataInternalAsync<TOutput>(this.conversionBuffer.AsMemory(0, minBufferSize),
                cancellationToken);
        }
    }
        ...
        where TOutput : unmanaged
    {
        ...
        public override void RaiseCallback(Span<byte> buffer, out int samplesWritten)
        {
            ...
            else
            {
                ...
                SampleConverter.InternalConvert<TInput, TOutput>(this.conversionBuffer.AsSpan(0, minBufferSize),
                    MemoryMarshal.Cast<byte, TOutput>(buffer));
            }
        }
    }
}
...
public delegate void BufferNeededCallback<TSample>(Span<TSample> buffer, out int samplesWritten)
