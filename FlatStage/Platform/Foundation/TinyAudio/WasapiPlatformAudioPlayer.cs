...
using FlatStage.Foundation.TinyAudio.Wasapi;

namespace FlatStage.Foundation.TinyAudio;

public sealed class WasapiPlatformAudioPlayer : PlatformAudioPlayer
{
    ...
    private WasapiPlatformAudioPlayer(AudioClient audioClient)
        : base(audioClient.MixFormat)
    {
        this.audioClient = audioClient;
    }

    public static WasapiPlatformAudioPlayer Create(TimeSpan bufferLength, bool useCallback = false)
    {
        var client = MediaDevice.Default.CreateAudioClient();
        try
        {
            client.Initialize(bufferLength, useCallback: useCallback);
            return new WasapiPlatformAudioPlayer(client);
        }
        catch
        {
            client.Dispose();
    ...
    protected override void Start(bool useCallback)
    {
        if (useCallback)
        {
            ...
            this.callbackWaitHandle =
                ThreadPool.UnsafeRegisterWaitForSingleObject(this.bufferReady, this.HandleBufferReady, null, -1, false);
        }
