...
internal sealed class AudioClient : IDisposable
{
    ...
    public void Initialize(TimeSpan bufferDuration, AudioFormat? audioFormat = null, bool useCallback = false)
    {
        ...
        unsafe
        {
            ...
            uint res = this.inst->Vtbl->Initialize(this.inst, 0, useCallback ? AUDCLNT_STREAMFLAGS_EVENTCALLBACK : 0,
                bufferDuration.Ticks, 0, (WAVEFORMATEX*)&wfx, &sessionId);
            if (res != 0)
                throw new InvalidOperationException();
    ...
    private static bool TryGetWaveFormat(AudioFormat format, out WAVEFORMATEXTENSIBLE wfx)
    {
        unsafe
        {
            ...
            else if (format.SampleFormat == SampleFormat.UnsignedPcm8 ||
                     format.SampleFormat == SampleFormat.SignedPcm16)
            {
