...
using FlatStage.Foundation.NVorbis.Contracts;
using FlatStage.Foundation.NVorbis.Contracts.Ogg;

namespace FlatStage.Foundation.NVorbis.Ogg;
...
class PacketProvider : Contracts.IPacketProvider, IPacketReader
{
    ...
    private (long lastPageGranulePos, int lastPagePacketLength, int firstRealPacket) GetPreviousPageInfo(int pageIndex,
        GetPacketGranuleCount getPacketGranuleCount)
    {
        if (pageIndex > 0)
        {
            int lastPagePacketLength;
            if (_reader.GetPage(pageIndex - 1, out var lastPageGranulePos, out _, out _, out var isContinued,
                    out var lastPacketCount, out _))
            {
                if (pageIndex > _reader.FirstDataPageIndex)
                {
                    ...
                    var lastPacket = CreatePacket(ref pageIndex, ref lastPacketIndex, false, 0, false, isContinued,
                        lastPacketCount, 0);
                    if (lastPacket == null)
                    {
    ...
    private (long[] gps, long endGP) GetTargetPageInfo(int pageIndex, int firstRealPacket, int lastPagePacketLength,
        GetPacketGranuleCount getPacketGranuleCount)
    {
        if (!_reader.GetPage(pageIndex, out var pageGranulePos, out var isResync, out var isContinuation,
                out var isContinued, out var packetCount, out _))
        {
        ...
        for (var i = packetCount - 1; i >= firstRealPacket; i--)
        {
            ...
            var packet = CreatePacket(ref pageIndex, ref i, false, pageGranulePos, i == 0 && isResync, isContinued,
                packetCount, 0);
            if (packet == null)
            {
                throw new System.IO.InvalidDataException("Could not find end of continuation!");
    ...
    private int FindPacket(int pageIndex, long[] gps, long endGP, long lastPageGranulePos, int lastPagePacketLength,
        ref long granulePos)
    {
    ...
    private int FindPacket(int pageIndex, int preRoll, ref long granulePos, GetPacketGranuleCount getPacketGranuleCount)
    {
        ...
        var (lastPageGranulePos, lastPagePacketLength, firstRealPacket) =
            GetPreviousPageInfo(pageIndex, getPacketGranuleCount);
    ...
    private bool NormalizePacketIndex(ref int pageIndex, ref int packetIndex)
    {
        ...
        while (pktIdx < (isContinuation ? 1 : 0))
        {
            ...
            if (!_reader.GetPage(--pgIdx, out _, out isResync, out isContinuation, out var isContinued,
                    out var packetCount, out _))
            {
                return false;
            }
    ...
    private Packet GetNextPacket(ref int pageIndex, ref int packetIndex)
    {
        ...
        if (_lastPacketPacketIndex != packetIndex || _lastPacketPageIndex != pageIndex || _lastPacket == null)
        {
            ...
            while (_reader.GetPage(pageIndex, out var granulePos, out var isResync, out _, out var isContinued,
                       out var packetCount, out var pageOverhead))
            {
                _lastPacketPageIndex = pageIndex;
                _lastPacketPacketIndex = packetIndex;
                _lastPacket = CreatePacket(ref pageIndex, ref packetIndex, true, granulePos, isResync, isContinued,
    ...
    private Packet CreatePacket(ref int pageIndex, ref int packetIndex, bool advance, long granulePos, bool isResync,
        bool isContinued, int packetCount, int pageOverhead)
    {
        ...
        if (isContinued && packetIndex == packetCount - 1)
        {
            ...
            while (isContinued)
            {
                if (!_reader.GetPage(++contPageIdx, out granulePos, out isResync, out var isContinuation,
                        out isContinued, out packetCount, out var contPageOverhead))
                {
