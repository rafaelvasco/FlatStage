...
class Mode : IMode
{
    ...
    private bool GetPacketInfo(IPacket packet, out int windowIndex, out int packetStartIndex, out int packetValidLength,
    ...
    public bool Decode(IPacket packet, float[][] buffer, out int packetStartindex, out int packetValidLength,
        out int packetTotalLength)
    {
        if (GetPacketInfo(packet, out var windowIndex, out packetStartindex, out packetValidLength,
