...
using FlatStage.Foundation.NVorbis.Ogg;

namespace FlatStage.Foundation.NVorbis;
...
public sealed class StreamDecoder : IStreamDecoder
{
    ...
    private bool ReadNextPacket(int bufferedSamples, out long? samplePosition)
    {
        ...
        var curPacket = DecodeNextPacket(out var startIndex, out var validLen, out var totalLen, out var isEndOfStream,
            out samplePosition, out var bitsRead, out var bitsRemaining, out var containerOverheadBits);
    ...
    private float[][] DecodeNextPacket(out int packetStartindex, out int packetValidLength, out int packetTotalLength,
        out bool isEndOfStream, out long? samplePosition, out int bitsRead, out int bitsRemaining,
    ...
    private static void OverlapBuffers(float[][] previous, float[][] next, int prevStart, int prevLen, int nextStart,
        int channels)
    {
