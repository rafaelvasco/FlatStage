...
public struct Color : IEquatable<Color>
{
    ...
    /// </summary>
    /// <param name="color">A <see cref="Vec4"/> representing a color.</param>
    public Color(Vec4 color)
    {
        _abgr = 0;

        R = (byte)Calc.Clamp(color.X * 255, byte.MinValue, byte.MaxValue);
        G = (byte)Calc.Clamp(color.Y * 255, byte.MinValue, byte.MaxValue);
        B = (byte)Calc.Clamp(color.Z * 255, byte.MinValue, byte.MaxValue);
        A = (byte)Calc.Clamp(color.W * 255, byte.MinValue, byte.MaxValue);
    }

    /// <summary>
    /// Constructs an RGBA color from the XYZW unit length components of a vector.
    /// </summary>
    /// <param name="color">A <see cref="Vec3"/> representing a color.</param>
    ...
    public Color(float r, float g, float b)
    {
        ...
        R = (byte)Calc.Clamp(r * 255, byte.MinValue, byte.MaxValue);
        G = (byte)Calc.Clamp(g * 255, byte.MinValue, byte.MaxValue);
        B = (byte)Calc.Clamp(b * 255, byte.MinValue, byte.MaxValue);
        A = 255;
    }
    ...
    {
        _abgr = 0;
        R = (byte)Calc.Clamp(r, byte.MinValue, byte.MaxValue);
        G = (byte)Calc.Clamp(g, byte.MinValue, byte.MaxValue);
        B = (byte)Calc.Clamp(b, byte.MinValue, byte.MaxValue);
        A = 255;
    }
    ...
    public Color(int r, int g, int b, int alpha)
    {
        ...
        G = (byte)Calc.Clamp(g, byte.MinValue, byte.MaxValue);
        B = (byte)Calc.Clamp(b, byte.MinValue, byte.MaxValue);
        A = (byte)Calc.Clamp(alpha, byte.MinValue, byte.MaxValue);
    }
    ...
    public Color(float r, float g, float b, float alpha)
    {
        ...
        G = (byte)Calc.Clamp(g * 255, byte.MinValue, byte.MaxValue);
        B = (byte)Calc.Clamp(b * 255, byte.MinValue, byte.MaxValue);
        A = (byte)Calc.Clamp(alpha * 255, byte.MinValue, byte.MaxValue);
    }
    ...
    public static Color FromHex(string hex)
    {
        ...
        if (!uint.TryParse(hex, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var packedValue))
            throw new ArgumentException("Hexadecimal string is not in the correct format.", nameof(hex));

        var a = (byte)packedValue;
    ...
    private static string ToRgbaHex(string hex)
    {
        ...
        if (hex.Length < 3 || hex.Length > 4) throw new ArgumentException("Invalid Hex Parameter Format");
    ...
    {
        amount = Calc.Clamp(amount, 0.0f, 1.0f);
        return new Color(
