...
public sealed class Synthesizer : IAudioRenderer
{
    ...
    public Synthesizer(string soundFontPath, int sampleRate) : this(new SoundFont(soundFontPath),
        new SynthesizerSettings(sampleRate))
    {
    }
    ...
    public Synthesizer(string soundFontPath, SynthesizerSettings settings) : this(new SoundFont(soundFontPath),
        settings)
    {
    }
    ...
    public void NoteOn(int channel, int key, int velocity)
    {
        ...
        if (!presetLookup.TryGetValue(presetId, out var preset))
        {
        ...
        {
            if (!presetRegion.Contains(key, velocity)) continue;

            foreach (var instrumentRegion in presetRegion.Instrument.RegionArray)
            {
                if (!instrumentRegion.Contains(key, velocity)) continue;
    ...
    private void RenderBlock()
    {
        ...
        if (enableReverbAndChorus)
        {
            ...
            foreach (var voice in voices)
            {
                var previousGain = reverb!.InputGain * voice.PreviousReverbSend *
                                   (voice.PreviousMixGainLeft + voice.PreviousMixGainRight);
                var currentGain = reverb!.InputGain * voice.CurrentReverbSend *
