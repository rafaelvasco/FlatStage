...
public ref struct Hasher
{
    ...
    public static Hasher Create() => new(M);
    ...
    public void Add(ReadOnlySpan<byte> data)
    {
        unchecked
        {
            ...
            if (numChunks > 0)
            {
                ...
                FlatException.Assert(numChunks == chunks.Length);
            ...
            FlatException.Assert(m_bitCount == 0);

            data = data.Slice(fastBytes);
            FlatException.Assert(data.Length >= 0 && data.Length <= 7);
