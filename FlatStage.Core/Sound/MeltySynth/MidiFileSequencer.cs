...
public sealed class MidiFileSequencer : IAudioRenderer
{
    ...
    {
        this.midiFile = midiFile ?? throw new ArgumentNullException(nameof(midiFile));
        this.loop = loop;
    ...
    public void Render(Span<float> left, Span<float> right)
    {
        ...
        while (wrote < left.Length)
        {
            if (blockWrote == synthesizer.BlockSize)
            {
                ...
                blockWrote = 0;
                currentTime +=
                    MidiFile.GetTimeSpanFromSeconds((double)speed * synthesizer.BlockSize / synthesizer.SampleRate);
            }
