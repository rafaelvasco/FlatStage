...
internal sealed class VolumeEnvelope
{
    ...
    private bool Process(int sampleCount)
    {
        ...
        switch (stage)
        {
            ...
            case Stage.Decay:
                value = Math.Max((float)SoundFontMath.ExpCutoff(decaySlope * (currentTime - decayStartTime)),
                    sustainLevel);
                priority = 1F + value;
                return value > SoundFontMath.NonAudible;

            case Stage.Release:
                value = (float)(releaseLevel *
                                SoundFontMath.ExpCutoff(releaseSlope * (currentTime - releaseStartTime)));
                priority = value;
                return value > SoundFontMath.NonAudible;
