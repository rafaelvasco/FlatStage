...
public struct RectF : IEquatable<RectF>
{
    ...
    /// <summary>
    /// Returns the X,Y coordinate of the TopLeft corner of this <see cref="RectF"/>
    /// </summary>
    public readonly Vec2 TopLeft => new(X, Y);

    /// <summary>
    /// Returns the X,Y coordinate of the TopRight corner of this <see cref="RectF"/>
    /// </summary>
    public readonly Vec2 TopRight => new(X + Width, Y);


    /// <summary>
    /// Returns the X,Y coordinate of the BottomLeft corner of this <see cref="RectF"/>
    /// </summary>
    public readonly Vec2 BottomLeft => new(X, Y + Height);

    /// <summary>
    /// Returns the X,Y coordinate of the BottomRight corner of this <see cref="RectF"/>
    /// </summary>
    public readonly Vec2 BottomRight => new(X + Width, Y + Height);
    ...
    )
    {
        ...
        {
            float rightSide = MathUtils.Min(
                value1.X + value1.Width,
                value2.X + value2.Width
            );
            float leftSide = MathUtils.Max(value1.X, value2.X);
            float topSide = MathUtils.Max(value1.Y, value2.Y);
            float bottomSide = MathUtils.Min(
    ...
    {
        float x = MathUtils.Min(value1.X, value2.X);
        float y = MathUtils.Min(value1.Y, value2.Y);
        return new RectF(
            x,
            y,
            MathUtils.Max(value1.Right, value2.Right) - x,
            MathUtils.Max(value1.Bottom, value2.Bottom) - y
        );
    }
    ...
    public static void Union(ref RectF value1, ref RectF value2, out RectF result)
    {
        ...
        result.Width = MathUtils.Max(value1.Right, value2.Right) - result.X;
        result.Height = MathUtils.Max(value1.Bottom, value2.Bottom) - result.Y;
    }
