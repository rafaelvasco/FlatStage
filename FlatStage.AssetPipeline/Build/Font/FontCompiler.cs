...
internal unsafe class FontCompiler : IDisposable
{
    ...
    private readonly FT_LibraryRec_* _ftLibrary;

    internal FontCompiler()
    {
        var lib = (FT_LibraryRec_*)Marshal.AllocHGlobal(Marshal.SizeOf(typeof(FT_LibraryRec_)));
    ...
    public FontData Build(string id, byte[] fontData, int initialBitmapSize, int fontSize, CharRange charRange, int padding)
    {
        ...
        for (int i = charRange.Start; i <= charRange.End; ++i)
        {
            ...
            int glyphW = (int)(face->glyph->metrics.width / 64);
            int glyphH = (int)(face->glyph->metrics.height / 64);

            if (glyphW <= 0 || glyphH <= 0)
            {
                glyphW = (int)((face->glyph->metrics.horiBearingX + face->glyph->metrics.horiAdvance) / 64);
                glyphH = (int)(face->glyph->metrics.vertBearingY / 64);
            }
        ...
        int bboxYmax = (int)(face->bbox.yMax / 64);
        ...
        foreach (var rect in glyphTargetRegionsOnBitmap)
        {
            ...
            int glyphWidth = (int)(face->glyph->metrics.width / 64);
            //int glyphHeight = face->glyph->metrics.height / 64;

            int advance = (int)(face->glyph->metrics.horiAdvance / 64);

            int xOffset = (advance - glyphWidth) / 2;
            int yOffset = (int)(bboxYmax - (face->glyph->metrics.horiBearingY / 64));

            if (FT.FT_Render_Glyph(face->glyph, FT_Render_Mode_.FT_RENDER_MODE_NORMAL) != FT_Error.FT_Err_Ok)
            {
